/* Toast Menus V2 client + transform.
 *
 * Pulls the live menu and REAL in-cafe prices straight from Toast so we stop
 * hand-maintaining pages/menu.js. Used by:
 *   - api/menu.js          (Vercel serverless function the browser calls)
 *   - scripts/sync-menu.mjs (optional: bake a static menu.json from the CLI)
 *
 * Auth: POST clientId/clientSecret to /authentication/v1/authentication/login
 *       with userAccessType TOAST_MACHINE_CLIENT. Token lives ~1h; we cache it.
 * Menus: GET /menus/v2/menus with the restaurant GUID in the
 *        Toast-Restaurant-External-ID header.
 *
 * Required env vars (set in Vercel, never committed):
 *   TOAST_API_HOST          e.g. https://ws-api.toasttab.com  (production)
 *   TOAST_CLIENT_ID
 *   TOAST_CLIENT_SECRET
 *   TOAST_RESTAURANT_GUID   the restaurant's externalId / GUID
 */

const AUTH_PATH = "/authentication/v1/authentication/login";
const MENUS_PATH = "/menus/v2/menus";
const ORDERS_PATH = "/orders/v2/orders";
const ORDERS_BULK_PATH = "/orders/v2/ordersBulk";

/* In-process caches (warm across invocations on a reused Vercel instance). */
let _token = null;          // { value, expiresAt (ms epoch) }
let _menuCache = null;      // { data, fetchedAt (ms epoch) }
let _configCache = null;    // { data (restaurant config), fetchedAt (ms epoch) }

const MENU_TTL_MS = 5 * 60 * 1000;        // serve cached menu for 5 min
const CONFIG_TTL_MS = 10 * 60 * 1000;     // serve cached hours/config for 10 min
const TOKEN_SKEW_MS = 60 * 1000;          // refresh token 1 min before expiry
const RESTAURANT_PATH = "/restaurants/v1/restaurants/";

function requireEnv(name, env) {
  const v = env[name];
  if (!v) throw new Error("Missing required env var: " + name);
  return v;
}

function host(env) {
  return requireEnv("TOAST_API_HOST", env).replace(/\/+$/, "");
}

/* ---------- Auth ---------- */
async function getToken(env, now) {
  if (_token && _token.expiresAt - TOKEN_SKEW_MS > now) return _token.value;

  const res = await fetch(host(env) + AUTH_PATH, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      clientId: requireEnv("TOAST_CLIENT_ID", env),
      clientSecret: requireEnv("TOAST_CLIENT_SECRET", env),
      userAccessType: "TOAST_MACHINE_CLIENT"
    })
  });

  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error("Toast auth failed (" + res.status + "): " + body.slice(0, 300));
  }

  const json = await res.json();
  // Response shape: { token: { accessToken, expiresIn, tokenType }, ... }
  const tok = json && json.token;
  if (!tok || !tok.accessToken) {
    throw new Error("Toast auth response missing token");
  }
  const expiresInMs = (Number(tok.expiresIn) || 3600) * 1000;
  _token = { value: tok.accessToken, expiresAt: now + expiresInMs };
  return _token.value;
}

/* ---------- Fetch raw menus ---------- */
async function fetchMenus(env, now) {
  const token = await getToken(env, now);
  const res = await fetch(host(env) + MENUS_PATH, {
    method: "GET",
    headers: {
      Authorization: "Bearer " + token,
      "Toast-Restaurant-External-ID": requireEnv("TOAST_RESTAURANT_GUID", env),
      Accept: "application/json"
    }
  });

  if (res.status === 401) {
    // Token may have been invalidated early; clear and retry once.
    _token = null;
    const retryToken = await getToken(env, now);
    const retry = await fetch(host(env) + MENUS_PATH, {
      method: "GET",
      headers: {
        Authorization: "Bearer " + retryToken,
        "Toast-Restaurant-External-ID": requireEnv("TOAST_RESTAURANT_GUID", env),
        Accept: "application/json"
      }
    });
    if (!retry.ok) {
      const body = await retry.text().catch(() => "");
      throw new Error("Toast menus fetch failed (" + retry.status + "): " + body.slice(0, 300));
    }
    return retry.json();
  }

  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error("Toast menus fetch failed (" + res.status + "): " + body.slice(0, 300));
  }
  return res.json();
}

/* ---------- Transform Toast -> ILLY_MENU shape ----------
 * The Menus V2 payload is a tree: menus[] -> menuGroups[] -> menuItems[].
 * Each menuItem has a `price` and optional nested groups for sizes/modifiers.
 * We flatten it into the category/item shape pages/menu.js already renders:
 *   categories: [{ id, name, items: [{ id, name, desc, price, sizes?, outOfStock }] }]
 * `overrides` (built from the existing menu.js) lets us re-attach local photos
 * and stable slugs that Toast doesn't know about, keyed by lowercased item name.
 */
function slugify(s) {
  return String(s || "")
    .toLowerCase()
    .replace(/&/g, "and")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 60) || "item";
}

function itemPrice(mi) {
  if (typeof mi.price === "number") return mi.price;
  // Some items price only through their size group; take the lowest option.
  const sizes = extractSizes(mi);
  if (sizes.length) return Math.min.apply(null, sizes.map((s) => s.price));
  return 0;
}

function extractSizes(mi) {
  const groups = mi.modifierGroups || mi.optionGroups || [];
  for (const g of groups) {
    const name = (g.name || "").toLowerCase();
    if (name.indexOf("size") === -1) continue;
    const opts = (g.modifierOptions || g.options || g.items || [])
      .map((o) => ({
        name: o.name,
        price: typeof o.price === "number" ? o.price : Number(o.basePrice) || 0
      }))
      .filter((o) => o.name);
    if (opts.length) return opts;
  }
  return [];
}

function isOutOfStock(mi) {
  // Toast surfaces 86'd items via visibility / inStock flags depending on config.
  if (mi.inStock === false) return true;
  if (Array.isArray(mi.visibility) && mi.visibility.length === 0) return true;
  return false;
}

function transform(raw, overrides) {
  overrides = overrides || {};
  const menus = Array.isArray(raw) ? raw : (raw && raw.menus) || [];
  const categories = [];
  const seenCat = {};

  for (const menu of menus) {
    const groups = menu.menuGroups || menu.groups || [];
    for (const g of groups) {
      const catName = g.name || "Menu";
      const catId = slugify(catName);
      let cat = seenCat[catId];
      if (!cat) {
        cat = { id: catId, name: catName, items: [] };
        seenCat[catId] = cat;
        categories.push(cat);
      }
      const items = g.menuItems || g.items || [];
      for (const mi of items) {
        if (!mi.name) continue;
        const ov = overrides[mi.name.toLowerCase()] || {};
        const sizes = extractSizes(mi);
        const item = {
          id: ov.id || slugify(mi.name),
          name: mi.name,
          price: itemPrice(mi)
        };
        // Toast GUIDs needed to place orders via the Orders API.
        if (mi.guid) item.guid = mi.guid;
        if (mi.itemGroupGuid || g.guid) item.itemGroupGuid = mi.itemGroupGuid || g.guid;
        if (mi.description) item.desc = mi.description;
        else if (ov.desc) item.desc = ov.desc;
        if (sizes.length) item.sizes = sizes;
        if (isOutOfStock(mi)) item.outOfStock = true;
        if (ov.photo) item.photo = ov.photo;
        if (ov.img) item.img = ov.img;
        cat.items.push(item);
      }
    }
  }
  return categories;
}

/* ---------- Public: get the menu (cached) ---------- */
async function getMenu(env, opts) {
  opts = opts || {};
  const now = opts.now || Date.now();
  if (!opts.force && _menuCache && now - _menuCache.fetchedAt < MENU_TTL_MS) {
    return { categories: _menuCache.data, cached: true, fetchedAt: _menuCache.fetchedAt };
  }
  const raw = await fetchMenus(env, now);
  const categories = transform(raw, opts.overrides);
  _menuCache = { data: categories, fetchedAt: now };
  return { categories, cached: false, fetchedAt: now };
}

/* ---------- Fetch restaurant config (hours, timezone, online ordering) ----------
 * GET /restaurants/v1/restaurants/{guid}. Returns the full config object; we only
 * use `general.timeZone`, `onlineOrdering.enabled`, and `schedules`
 * (weekSchedule -> daySchedule -> services[].hours). Cached 10 min.
 */
async function fetchRestaurantConfig(env, now) {
  const token = await getToken(env, now);
  const gid = requireEnv("TOAST_RESTAURANT_GUID", env);
  const res = await fetch(host(env) + RESTAURANT_PATH + gid, {
    method: "GET",
    headers: {
      Authorization: "Bearer " + token,
      "Toast-Restaurant-External-ID": gid,
      Accept: "application/json"
    }
  });
  if (!res.ok) {
    const body = await res.text().catch(function () { return ""; });
    const e = new Error("Toast restaurant config failed (" + res.status + "): " + body.slice(0, 300));
    e.code = "toast_error";
    e.status = res.status;
    throw e;
  }
  return res.json();
}

/* Day-of-week the restaurant config keys weekSchedule by. */
const WEEKDAY_KEYS = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];

/* Local {dateStr, day(0-6), minutes} for an epoch ms in a named IANA zone. */
function localPartsInZone(epochMs, timeZone) {
  const fmt = new Intl.DateTimeFormat("en-US", {
    timeZone: timeZone,
    weekday: "short", year: "numeric", month: "2-digit", day: "2-digit",
    hour: "2-digit", minute: "2-digit", hour12: false
  });
  const map = {};
  fmt.formatToParts(new Date(epochMs)).forEach(function (p) { map[p.type] = p.value; });
  const dayNames = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
  let hour = parseInt(map.hour, 10);
  if (hour === 24) hour = 0;
  return {
    dateStr: map.year + "-" + map.month + "-" + map.day,
    day: dayNames[map.weekday],
    minutes: hour * 60 + parseInt(map.minute, 10)
  };
}

/* "HH:MM:SS.mmm" or "HH:MM" -> minutes from midnight (null if unparseable). */
function hhmmToMin(s) {
  const m = String(s || "").match(/^(\d{1,2}):(\d{2})/);
  if (!m) return null;
  return parseInt(m[1], 10) * 60 + parseInt(m[2], 10);
}

function fmtMin(min) {
  let h = Math.floor(min / 60), m = min % 60;
  const ampm = h >= 12 ? "PM" : "AM";
  h = h % 12; if (h === 0) h = 12;
  return h + ":" + (m < 10 ? "0" + m : m) + " " + ampm;
}

/* Pull the [open, close] minute window for a given local day from the config's
 * schedules. Uses the day's `openTime`/`closeTime` when present, else the span
 * of its services[].hours. Returns null if the day has no schedule (closed). */
function dayWindowFromConfig(config, dayIndex) {
  const schedules = config && config.schedules;
  if (!schedules || !schedules.weekSchedule || !schedules.daySchedules) return null;
  const dayId = schedules.weekSchedule[WEEKDAY_KEYS[dayIndex]];
  if (!dayId) return null;
  const ds = schedules.daySchedules[dayId];
  if (!ds) return null;

  let open = hhmmToMin(ds.openTime);
  let close = hhmmToMin(ds.closeTime);
  if (open == null || close == null) {
    // Derive from the service blocks (earliest start, latest end).
    (ds.services || []).forEach(function (svc) {
      const s = hhmmToMin(svc.hours && svc.hours.startTime);
      const e = hhmmToMin(svc.hours && svc.hours.endTime);
      if (s != null) open = open == null ? s : Math.min(open, s);
      if (e != null) close = close == null ? e : Math.max(close, e);
    });
  }
  if (open == null || close == null) return null;
  return { open: open, close: close };
}

/* ---------- Public: live availability from Toast hours (cached) ----------
 * Mirrors lib/hours.availabilityAt() but sourced from the live Toast schedule.
 * `lastOrderBeforeCloseMin` keeps the kitchen wind-down buffer (default 20).
 * Returns { open, reason, opensAt, lastOrder, source:"toast", timeZone }.
 * Throws if Toast is unreachable so the caller can fall back to local config.
 */
async function getAvailability(env, opts) {
  opts = opts || {};
  const now = opts.now || Date.now();
  const lastOrderBefore =
    typeof opts.lastOrderBeforeCloseMin === "number" ? opts.lastOrderBeforeCloseMin : 20;

  let config;
  if (!opts.force && _configCache && now - _configCache.fetchedAt < CONFIG_TTL_MS) {
    config = _configCache.data;
  } else {
    config = await fetchRestaurantConfig(env, now);
    _configCache = { data: config, fetchedAt: now };
  }

  const tz = (config.general && config.general.timeZone) || "America/New_York";

  // If the café has online ordering switched off in Toast, treat as closed.
  if (config.onlineOrdering && config.onlineOrdering.enabled === false) {
    return { open: false, reason: "Online ordering is currently turned off.", opensAt: null, source: "toast", timeZone: tz };
  }

  const p = localPartsInZone(now, tz);
  const win = dayWindowFromConfig(config, p.day);
  if (!win) {
    return { open: false, reason: "Closed today.", opensAt: null, source: "toast", timeZone: tz };
  }

  const lastOrderMin = win.close - lastOrderBefore;
  if (p.minutes < win.open) {
    return {
      open: false,
      reason: "Online ordering opens at " + fmtMin(win.open) + ".",
      opensAt: fmtMin(win.open),
      source: "toast", timeZone: tz
    };
  }
  if (p.minutes >= lastOrderMin) {
    return {
      open: false,
      reason: "Online ordering is closed for today (last order " + fmtMin(lastOrderMin) + ").",
      opensAt: null,
      source: "toast", timeZone: tz
    };
  }
  return {
    open: true,
    reason: "Accepting online orders until " + fmtMin(lastOrderMin) + ".",
    opensAt: fmtMin(win.open),
    lastOrder: fmtMin(lastOrderMin),
    source: "toast", timeZone: tz
  };
}

/* ---------- Place an order (pay-at-pickup, UNPAID) ----------
 * Creates a real order in the café POS via the Orders API. We deliberately do
 * NOT attach a payment — the customer pays at the counter on pickup, so the site
 * never handles card data (no PCI scope). Toast surfaces it to staff as an
 * unpaid online/takeout order.
 *
 * `order` shape (from the browser, validated by the caller):
 *   {
 *     items: [{ guid, itemGroupGuid, quantity }],   // GUIDs from the menu fetch
 *     customer: { firstName, lastName, phone, email },
 *     note: "optional kitchen note"
 *   }
 *
 * Required env: TOAST_RESTAURANT_GUID, plus a dining option + revenue center
 * GUID for takeout, which Toast assigns per restaurant:
 *   TOAST_DINING_OPTION_GUID   (takeout dining option)
 *   TOAST_REVENUE_CENTER_GUID  (optional; some configs require it)
 */
function buildOrderPayload(order, env) {
  const selections = (order.items || []).map(function (it) {
    const sel = {
      item: { guid: it.guid },
      quantity: Math.max(1, parseInt(it.quantity, 10) || 1)
    };
    if (it.itemGroupGuid) sel.itemGroup = { guid: it.itemGroupGuid };
    if (Array.isArray(it.modifiers) && it.modifiers.length) {
      sel.modifiers = it.modifiers
        .filter(function (m) { return m && m.guid; })
        .map(function (m) {
          return {
            item: { guid: m.guid },
            quantity: Math.max(1, parseInt(m.quantity, 10) || 1),
            itemGroup: m.itemGroupGuid ? { guid: m.itemGroupGuid } : undefined
          };
        });
    }
    return sel;
  });

  const payload = {
    // Takeout pickup order, entered via API.
    diningOption: { guid: requireEnv("TOAST_DINING_OPTION_GUID", env) },
    checks: [
      {
        selections: selections,
        customer: order.customer
          ? {
              firstName: order.customer.firstName || "",
              lastName: order.customer.lastName || "",
              phone: order.customer.phone || "",
              email: order.customer.email || ""
            }
          : undefined
      }
    ]
  };
  if (order.note) payload.checks[0].note = order.note;

  if (env.TOAST_REVENUE_CENTER_GUID) {
    payload.revenueCenter = { guid: env.TOAST_REVENUE_CENTER_GUID };
  }
  return payload;
}

async function createOrder(order, env, opts) {
  opts = opts || {};
  const now = opts.now || Date.now();

  if (!order || !Array.isArray(order.items) || !order.items.length) {
    const e = new Error("order has no items");
    e.code = "empty_order";
    throw e;
  }
  const missingGuid = order.items.some(function (it) { return !it.guid; });
  if (missingGuid) {
    const e = new Error("one or more items is missing a Toast guid");
    e.code = "missing_guid";
    throw e;
  }

  const token = await getToken(env, now);
  const payload = buildOrderPayload(order, env);

  const res = await fetch(host(env) + ORDERS_PATH, {
    method: "POST",
    headers: {
      Authorization: "Bearer " + token,
      "Toast-Restaurant-External-ID": requireEnv("TOAST_RESTAURANT_GUID", env),
      "Content-Type": "application/json",
      Accept: "application/json"
    },
    body: JSON.stringify(payload)
  });

  if (!res.ok) {
    const body = await res.text().catch(function () { return ""; });
    const e = new Error("Toast order create failed (" + res.status + "): " + body.slice(0, 400));
    e.code = "toast_error";
    e.status = res.status;
    throw e;
  }
  return res.json();
}

/* ---------- Bulk orders (for admin metrics) ----------
 * GET /orders/v2/ordersBulk?startDate=&endDate=&page=&pageSize=
 * Returns full orders (checks -> selections -> payments) for a UTC time window.
 * Dates are ISO 8601. Toast paginates; we walk pages until a short page or the
 * safety cap is hit. Used only by admin endpoints — never the public site.
 */
function toIso(d) {
  if (d instanceof Date) return d.toISOString();
  return new Date(d).toISOString();
}

async function fetchOrdersPage(env, token, startIso, endIso, page, pageSize) {
  const qs =
    "?startDate=" + encodeURIComponent(startIso) +
    "&endDate=" + encodeURIComponent(endIso) +
    "&page=" + page +
    "&pageSize=" + pageSize;
  const res = await fetch(host(env) + ORDERS_BULK_PATH + qs, {
    method: "GET",
    headers: {
      Authorization: "Bearer " + token,
      "Toast-Restaurant-External-ID": requireEnv("TOAST_RESTAURANT_GUID", env),
      Accept: "application/json"
    }
  });
  if (!res.ok) {
    const body = await res.text().catch(function () { return ""; });
    const e = new Error("Toast ordersBulk failed (" + res.status + "): " + body.slice(0, 300));
    e.code = "toast_error";
    e.status = res.status;
    throw e;
  }
  const json = await res.json();
  return Array.isArray(json) ? json : (json && json.orders) || [];
}

async function getOrdersBulk(env, opts) {
  opts = opts || {};
  const now = opts.now || Date.now();
  const startIso = toIso(opts.start);
  const endIso = toIso(opts.end || now);
  const pageSize = Math.min(100, Math.max(1, opts.pageSize || 100));
  const maxPages = Math.max(1, opts.maxPages || 20); // safety cap (~2000 orders)

  const token = await getToken(env, now);
  const all = [];
  for (let page = 1; page <= maxPages; page++) {
    const batch = await fetchOrdersPage(env, token, startIso, endIso, page, pageSize);
    if (!batch.length) break;
    all.push.apply(all, batch);
    if (batch.length < pageSize) break;
  }
  return all;
}

/* Reduce raw Toast orders into the metrics the admin dashboard renders.
 * - excludes voided/deleted orders and voided checks
 * - revenue is the sum of check amounts (net of voids), in dollars
 * - itemCounts aggregates sold quantities by display name for "top sellers"
 * - buckets orders by local day for the trend sparkline
 */
function summarizeOrders(orders, opts) {
  opts = opts || {};
  const tzOffsetMin = typeof opts.tzOffsetMin === "number" ? opts.tzOffsetMin : 0;
  let orderCount = 0;
  let revenueCents = 0;
  let paidCents = 0;
  let refundedCents = 0;
  let guestCount = 0;
  const itemCounts = {};   // name -> { qty, revenueCents }
  const byDay = {};        // YYYY-MM-DD -> { orders, revenueCents }
  const byHour = new Array(24).fill(0);

  function localParts(ms) {
    const d = new Date(ms - tzOffsetMin * 60 * 1000);
    const day = d.toISOString().slice(0, 10);
    return { day: day, hour: d.getUTCHours() };
  }

  (orders || []).forEach(function (o) {
    if (!o || o.voided || o.deleted) return;
    const openedMs = o.openedDate ? Date.parse(o.openedDate) : (o.createdDate ? Date.parse(o.createdDate) : NaN);
    const checks = (o.checks || []).filter(function (c) { return c && !c.voided && !c.deleted; });
    if (!checks.length) return;

    orderCount += 1;
    if (typeof o.numberOfGuests === "number") guestCount += o.numberOfGuests;

    let orderRevenue = 0;
    checks.forEach(function (c) {
      const amt = typeof c.totalAmount === "number" ? c.totalAmount
        : (typeof c.amount === "number" ? c.amount : 0);
      orderRevenue += amt;
      (c.payments || []).forEach(function (p) {
        if (!p) return;
        const pAmt = (typeof p.amount === "number" ? p.amount : 0) + (typeof p.tipAmount === "number" ? p.tipAmount : 0);
        if (p.paymentStatus === "VOIDED" || p.refundStatus === "FULL") {
          refundedCents += Math.round(pAmt * 100);
        } else {
          paidCents += Math.round(pAmt * 100);
        }
      });
      (c.selections || []).forEach(function (s) {
        if (!s || s.voided) return;
        const name = (s.displayName || s.name || "Item").trim();
        const qty = typeof s.quantity === "number" ? s.quantity : 1;
        const lineCents = Math.round((typeof s.price === "number" ? s.price : 0) * 100);
        const bucket = itemCounts[name] || (itemCounts[name] = { qty: 0, revenueCents: 0 });
        bucket.qty += qty;
        bucket.revenueCents += lineCents;
      });
    });

    revenueCents += Math.round(orderRevenue * 100);

    if (!Number.isNaN(openedMs)) {
      const parts = localParts(openedMs);
      const day = byDay[parts.day] || (byDay[parts.day] = { orders: 0, revenueCents: 0 });
      day.orders += 1;
      day.revenueCents += Math.round(orderRevenue * 100);
      byHour[parts.hour] += 1;
    }
  });

  const topSellers = Object.keys(itemCounts)
    .map(function (name) { return { name: name, qty: itemCounts[name].qty, revenueCents: itemCounts[name].revenueCents }; })
    .sort(function (a, b) { return b.qty - a.qty; })
    .slice(0, 8);

  const days = Object.keys(byDay).sort().map(function (day) {
    return { day: day, orders: byDay[day].orders, revenueCents: byDay[day].revenueCents };
  });

  const avgOrderCents = orderCount ? Math.round(revenueCents / orderCount) : 0;

  return {
    orderCount: orderCount,
    revenueCents: revenueCents,
    paidCents: paidCents,
    refundedCents: refundedCents,
    avgOrderCents: avgOrderCents,
    guestCount: guestCount,
    topSellers: topSellers,
    days: days,
    byHour: byHour
  };
}

function _resetCachesForTest() {
  _token = null;
  _menuCache = null;
  _configCache = null;
}

module.exports = {
  getMenu,
  getToken,
  fetchMenus,
  transform,
  slugify,
  createOrder,
  buildOrderPayload,
  getOrdersBulk,
  summarizeOrders,
  getAvailability,
  fetchRestaurantConfig,
  dayWindowFromConfig,
  _resetCachesForTest,
  MENU_TTL_MS,
  CONFIG_TTL_MS
};
